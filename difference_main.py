...
if args.mode in ["test","both"]:
    ...
    # ADDED DIFFERENCE DISTANCE STRATEGY
    # test_score = pd.Series(test_gda_result.min(axis=1))
    min_test_score = pd.Series(test_gda_result.min(axis=1))
    max_test_score = pd.Series(test_gda_result.max(axis=1))
    test_score = max_test_score - min_test_score # difference
    ###########
