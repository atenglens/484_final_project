...
@Model.register("elmo_st")
class NSDSlotTaggingModel(Model):
    
    def __init__(self, 
                 vocab: Vocabulary,
                 elmo_embedder: Optional[ElmoTokenEmbedder] = None,
        ...
        if elmo_embedder:
            self.use_elmo = True
            self.bert_embedder = elmo_embedder
        else:
            self.use_elmo = False
        ...
        if encoder:
            ...
            hidden2tag_in_dim = elmo_embedder.get_output_dim()
    ...
    def forward(self,
        ...
        if self.use_elmo:
            embeddings = self.elmo_embedder(sentence["bert"], sentence["bert-offsets"], sentence["bert-type-ids"])
            if self.dropout:
