...
@Model.register("openai_st")
class NSDSlotTaggingModel(Model):
    
    def __init__(self, 
                 vocab: Vocabulary,
                 openai_embedder: Optional[OpenaiTransformerEmbedder] = None,
        ...
        if openai_embedder:
            ...
            self.basic_embedder = BasicTextFieldEmbedder({
                "tokens": Embedding(vocab.get_vocab_size(namespace="tokens"), 768)
            })
        ...
        if encoder:
            ...
            hidden2tag_in_dim = openai_embedder.get_output_dim()
    ...
    def forward(self,
        ...
        if self.use_openai:
            embeddings = self.openai_embedder(sentence["openai_transformer"], sentence["openai_transformer-offsets"])
            if self.dropout:
